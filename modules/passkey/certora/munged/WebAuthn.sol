...
library WebAuthn {
    ...
    function castSignature(bytes calldata signature) internal pure returns (bool isValid, Signature calldata data) {
        ...
        assembly ("memory-safe") {
            ...
            authenticatorDataLength := calldataload(add(data, t1))
            let t2 := calldataload(add(data, 0x20))
            isValid := and(isValid, lt(t2, signature.length))
        ...
        unchecked {
            ...
            // MUNGED To simplify prover execution
            isValid = isValid && signature.length < 193 + authenticatorDataAlignedLength + clientDataFieldsAlignedLength;
        }
    }
