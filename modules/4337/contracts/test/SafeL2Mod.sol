...
contract SafeL2Mod is SafeL2 {
    using SafeMath for uint256;

    function checkSignatures(bytes32 dataHash, bytes memory data, bytes memory signatures) public view override {
    ...
    function checkNSignatures(
        ...
        require(signatures.length >= offset, "GS020");
        ...
        for (i = 0; i < requiredSignatures; i++) {
            ...
            if (v == 0) {
                ...
                offset = uint256(s).add(32).add(contractSignatureLen);
                require(offset <= signatures.length, "GS023");
        ...
        require(signatures.length == offset, "Invalid signature length");
    }
}
