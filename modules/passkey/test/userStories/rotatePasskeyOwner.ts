...
import { buildSafeUserOpTransaction, buildPackedUserOperationFromSafeUserOperation, signSafeOp } from '@safe-global/safe-4337/src/utils/userOp'
...
describe.only('Rotate passkey owner [@User story]', () => {
  ...
  it('should execute a userOp with WebAuthn signer as owner', async () => {
    const { user, proxyFactory, safeModuleSetup, module, entryPoint, verifier, singleton, navigator, SafeL2, signer,signerFactory, credential } = await setupTests()
    ...
    await (
      await entryPoint.handleOps([{ ...packedUserOp, signature: ethers.solidityPacked(['uint48', 'uint48', 'bytes'], [safeOp.validAfter, safeOp.validUntil, signature]),
    }], user.address)
    ).wait()
    ...
    await (
      await entryPoint.handleOps([{ ...packedUserOpSwapSigner, signature: ethers.solidityPacked(['uint48', 'uint48', 'bytes'], [safeOpSwapSigner.validAfter, safeOpSwapSigner.validUntil, signatureSwapSigner]),
    }], user.address)
    ).wait()
